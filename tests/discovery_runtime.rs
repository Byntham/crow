...
async fn discovered_yarn_workspace_inherits_root_setup_and_runs_pnp_child_offline() {
    ...
    .unwrap();
    std::fs::write(repo.join("packages/app/package.json"),r#"{"name":"app","version":"1.0.0","private":true,"dependencies":{"shared":"workspace:*"},"scripts":{"test":"node test.cjs"}}"#).unwrap();
    std::fs::write(
...
"#,
    ...
    assert_eq!(setup["status"], "passed", "{setup}");
    let result=call(&exec,"run_experiment",json!({"revision":"head","environment":setup["id"],"command":format!("cd /workspace/{} && {}",child["directory"].as_str().unwrap(),child["test"].as_str().unwrap())})).await;
    assert_eq!(result["status"], "passed", "{result}");
    assert!(
        result["stdout"]
            .as_str()
            .unwrap()
            .contains("workspace child used inherited Yarn and PnP dependency offline")
    );
    println!(
        "Discovered Yarn workspace inherited root manager and root setup; child PnP dependency and nested Yarn command passed offline."
    );
}
