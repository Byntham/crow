...
Node workspace members inherit their root's package manager and setup command. Discovery returns `setupDirectory` for installation and `directory` for the project's test/start commands. Explicit package-manager pins or lockfiles keep independent nested projects separate. Literal workspace paths, `*` and `**` are supported. Other patterns and pnpm workspace YAML require inspection; discovery leaves those command candidates empty instead of guessing npm.
