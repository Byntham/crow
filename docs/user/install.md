...
The bootstrap needs common Linux tools including `curl`, `tar`, and `sha256sum`. It explains missing prerequisites before installation. Crow setup offers installation of missing supported dependencies such as Git, GitHub CLI, Codex, Podman and its rootless helpers, and the chosen ingress client. Runtime testing is enabled by default. On Ubuntu/Debian, setup offers to install missing container packages with sudo, checks host support, and configures Crow’s service for container resource controls. If the host cannot support runtime testing, setup explains the failure and offers to continue with it disabled. See [runtime setup and limits](runtime-experiments.md). GitHub login happens during setup to connect your repositories, not to download Crow.

On a headless machine, open the printed browser URLs on your desktop. The installer reads setup prompts from your terminal even when invoked through the pipe above. Without a terminal, it installs only and prints the command to start setup later. Setup checks connections and configuration without running a PR review. It checks the listening port before creating a public route and checks the systemd user session before account onboarding. Rerunning setup retains valid worker pairing and supported model choices. If an existing Codex executable lacks required capabilities, setup offers a Crow-managed installation without replacing the user-managed executable.
