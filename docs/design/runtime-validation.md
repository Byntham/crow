...
The managed-environment and native screenshot implementation was also exercised with real npm, PyPI, Cargo and Go module downloads followed by offline tests, along with snapshot reuse, pinned-source restoration, browser artifacts and the original isolation/cancellation suite. Three model-driven reviews without Crow-specific repository instructions covered a visual defect, a clean visual change, and a Python regression with an existing test failure. See the [saved reports and screenshots](../validation/autonomous-runtime.md) and [reproduction guide](autonomous-runtime.md).
