...
Each installation owns its own GitHub App, registered during setup or connected from an existing App. Crow guides registration through GitHub's manifest flow and captures generated credentials after browser confirmation. App installation and repository selection remain visible operator choices. No Marketplace listing or manual copying of App keys is required by the registration flow. Connecting an existing App requires its ID and a private-key PEM file. Crow validates permissions and events, asks before replacing its webhook destination and secret, and saves pending credentials before the remote change so interrupted connections can resume. One App must have only one active Crow service; workers pair with that service and receive repository-scoped source-read tokens.
